...
export async function POST(request: NextRequest) {
  ...
  if (!asset || typeof asset !== 'string') {
    return NextResponse.json({ success: false, error: 'Asset is required' }, { status: 422 })
  }

  if (!paymentMethodId || typeof paymentMethodId !== 'string') {
    return NextResponse.json({ success: false, error: 'Payment method is required' }, { status: 422 })
  }

  if (!amount || typeof amount !== 'number' || amount <= 0) {
    return NextResponse.json({ success: false, error: 'Amount must be greater than zero' }, { status: 422 })
  }

  if (typeof fiatAmount !== 'number' || fiatAmount <= 0) {
    return NextResponse.json({ success: false, error: 'Fiat amount is required' }, { status: 422 })
  }

  const method = financeServices.offRamp.getMethods().find((method) => method.id === paymentMethodId)
  if (!method) {
  ...
  try {
    const result = await financeServices.offRamp.initiateWithdrawal(amount, asset, paymentMethodId, method.currency)
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Unable to process off-ramp' },
      { status: 500 },
    )
  }
}
